<template>
  <div id="dota2">
    ...
    <div class="mall-tools">
      <span class="mall-tools-title"> > 宝箱</span>
      <div class="mall-tools-list">
        <ul>
          <li v-for="(item,index) in treasure_list" ref="thisli" :data-index="index" :data-name="item.product_name" :key="item.id" :data-gameId="item.game_id" :data-price="item.cprice" :data-quality="item.quality" :data-icon="item.game_id" >
...
//        })
    ...
    mounted () {
      if(!this.$store.state.shop.treasure_list){
        this.$store.dispatch('treasureBoxDis')
      }
//      this.$store.dispatch('userInfo')

    },
    computed:{
      treasure_list:function () {
        return this.$store.state.shop.treasure_list?this.$store.state.shop.treasure_list:null
      },
      isShow:function(){
